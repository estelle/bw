jQuery( document ).ready(function( $ ) {
	...
	$( '.home .et_pb_container').on( "click", function() {
		$this = $(this);
		$thisLink = $this.find('.et_pb_more_button').attr('href');
		//alert('click test: ' + $thisLink);
		window.location.href = $thisLink;
	});
	
	function checkTextLength() {
		if($('h6.peopleText').length > 0){
			if(window.innerWidth < 380){
				$('h6.peopleText').each(function( index ) {
					var thisText = $( this ).text(); //.text();
				  	var thisTextLength = thisText.length;
					if(thisTextLength > 26){
						thisText = thisText.substring(0, 26) + '...';
					}
					$( this ).text(thisText);
				  	//console.log( index + ": " + $( this ).text() + ' ' + thisTextLength);
				});
				$('h5.peopleText').each(function( index ) {
					var thisText = $( this ).text(); //.text();
				  	var thisTextLength = thisText.length;
					if(thisTextLength > 26){
						thisText = thisText.substring(0, 26) + '...';
				...
				$('h6.peopleText').each(function( index ) {
					var thisText = $( this ).text(); //.text();
				  	var thisTextLength = thisText.length;
					if(thisTextLength > 45){
						thisText = thisText.substring(0, 45) + '...';
					}
					$( this ).text(thisText);
				  	//console.log( index + ": " + $( this ).text() + ' ' + thisTextLength);
				});
				...
				$('h6.peopleText').each(function( index ) {
					var thisText = $( this ).text(); //.text();
				  	var thisTextLength = thisText.length;
					if(thisTextLength > 55){
						thisText = thisText.substring(0, 55) + '...';
					}
					$( this ).text(thisText);
				  	//console.log( index + ": " + $( this ).text() + ' ' + thisTextLength);
				});
...
jQuery(document).ready(function($) {
	...
	if($('.home').length){
		...
		$(".profile-box-box").hover(function() {
	...
	if($('.page-id-144').length){
		...
		$(".noImagePresent").hover(function() { 
			$(this).find(".profile-box-highlight").css("display", "block"); 
			$(this).find(".peopleText").css("color", "#fff");
		}, 
		function() { 
			$(".profile-box-highlight").css("display", "none"); 
			$(this).find(".peopleText").css({"color": "#008FB4","display": "block"});
		}); 
