  ...
  <div class="profile-container">
    <div class="profile-column"> 
		<img class="profile-image" src="<?php echo $photo; ?>"/>
      	<div class="periwinkleBg profile-block">
...
?>
    ...
    <div class="profile-content person-profile">
...
<?php  
	...
	$j = 0;
		...
		foreach( $selectedProjects as $selectedProject){
			...
			if($j == 0){
				echo '<ul class="custom-block-container">';
			}
			...
			$readMore = get_field('read_more',$thisProject);
				...
				echo '<li class="custom-block">'; 
				if(!empty($featured_image)){
					
					if($readMore == 1){echo '<a href="' . get_permalink($selectedProject->ID) . '">';}
					
					echo '<div id="' . $post->ID . '" class="projectHoverImg profile-box" style="';
					if($readMore == 1){echo 'cursor:pointer; ';}
					
					echo 'background:url(' . $featured_image . ') 50% 50% no-repeat;">
	...
	<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width: 100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>
							<div class="custom-block-text-container">
								...
								<h6 class="peopleText" style="color: rgb(255, 255, 255); margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px; min-height: 0px !important padding-right: 10px; display: none; font-weight: 200;text-overflow: ellipsis;">' . get_the_title($selectedProject->ID) . '</h6>
	...
	if($readMore == 1){echo '</a>';}
					...
					echo '<div id="' . $post->ID . '" class="projectHoverImg profile-box noImagePresent" style="';
					if($readMore == 1){echo 'cursor:pointer; ';}
					
					echo 'background-color:rgba(191, 204, 215, 0.4);">
	
	<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width: 100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>
							<div class="custom-block-text-container">
								<h5 class="peopleText" style="font-weight: 600; color: #2b92b5; margin: 12px 20px 0px 20px; font-size: 14px; display: block;">' . get_field('client',$selectedProject->ID) . '</h5>
								<h6 class="peopleText" style="color: #2b92b5; margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px; min-height: 0px !important padding-right: 10px; display: block; font-weight: 200;text-overflow: ellipsis;">' . get_the_title($selectedProject->ID) . '</h6>
								
							</div>';
							
							if($readMore == 1){
	
	echo '<div class="peopleText" style="position: absolute; bottom: 13px; color: #2b92b5; font-size: 12px; display: block; font-family: museo-sans, sans-serif; font-weight: 300;">Read more <i class="fa fa-angle-right fa-fw"></i></div>';
							}
