...
function et_divi_load_scripts_styles(){
	...
	wp_enqueue_script( 'modernizr', $template_dir . '/js/modernizr-custom.js', array( 'jquery' ), $theme_version, true );
...
//--> CUSTOM TAXONOMIES FROM PREVIOUS THEME <--//
...
//--> END - CUSTOM TAXONOMIES FROM PREVIOUS THEME <--//
...
//--> BRIGHTWORKS EDIT - commented out <--//
//add_action( 'init', 'et_pb_register_posttypes', 0 );
//--> END - BRIGHTWORKS EDIT - commented out <--//
...
//--> CUSTOM BRIGHTWORKS FUNCTIONS <--//


function registerCustomScripts() {
	wp_enqueue_style('font_awesome', '//maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css', false, null);
	wp_enqueue_style('legacy_css', get_template_directory_uri() . '/css/legacy.css', false, null);
	wp_enqueue_style('pure_drawer_css', get_template_directory_uri() . '/css/pure-drawer.css', false, null);
	wp_enqueue_style('brightworks_custom_css', get_template_directory_uri() . '/css/brightworksCustom.css', false, null);
	wp_enqueue_style('brightworks_resonsive_css', get_template_directory_uri() . '/css/brightworksResponsive.css', false, null);
	wp_register_script('jquery_cookie_js', get_template_directory_uri() . '/js/jquery.cookie.js', 'jquery', false);
	wp_enqueue_script('jquery_cookie_js');
	wp_register_script('brightworks_custom_js', get_template_directory_uri() . '/js/brightworksCustom.js', 'jquery', false);
	wp_enqueue_script('brightworks_custom_js');
}
	
add_action('wp_enqueue_scripts', 'registerCustomScripts');	


function the_excerpt_max_charlength($copy, $charlength) {
	$charlength++;
	$ellipses = '...';
	
	if ( mb_strlen( $copy ) > $charlength ) {
		$subex = mb_substr( $copy, 0, $charlength - 5 );
		$exwords = explode( ' ', $subex );
		$excut = - ( mb_strlen( $exwords[ count( $exwords ) - 1 ] ) );
		if ( $excut < 0 ) {
			return mb_substr( $subex, 0, $excut ) . $ellipses;
		} else {
			return $subex . $ellipses;
		}
	} else {
		return $copy;
	}
...
function custom_clients_section_function(){
	$output = '';
	$output .= '<div class="sevices-section">';
	$sector = new WP_Query(array(
	...
	$sector_client_array_count = count($sector_client_array);
	$sector_client_array_1 = array_slice($sector_client_array, 0, round($sector_client_array_count / 2));
	$sector_client_array_2 = array_slice($sector_client_array, round($sector_client_array_count / 2));
	...
	if($layoutStyle == "Three Projects" || $layoutStyle == "Four Projects"){
    	$output .= '<div class="clients-block-container">';
			$output .= '<div style="" class="services-block">';
				$output .= '<div style="float:left; width:100%">';
					$output .= '<p><a href="/" class="back-to-top-btn">Back to Top</a></p>';
					$output .= '<div class="block-first-col">';
						//apply preferred heading style
						if(empty($heading_style)){
							$output .= '<h2 class="heading">' . get_the_title() . '</h2>';
						}
						else{
							$output .= '<' . $heading_style . ' class="customH heading">' . get_the_title() . '</' . $heading_style . '>';
						}
					$output .= '</div>';
					$output .= '<div class="services-description-container">';
					$output .= '	<div class="services-description-column">';
					if(empty($text_style)){
						$output .= '<p>';
						foreach($sector_client_array_1 as $sector_client):
							$output .= '<span class="clientList">' . $sector_client . '</span>';
					    endforeach;
						$output .= '</p>';
					}else{
						$output .= '<' . $text_style . ' class="customH">';
						foreach($sector_client_array_1 as $sector_client):
							$output .= '<span class="clientList">' . $sector_client . '</span>';
					    endforeach;
						$output .= '</' . $text_style . '>';
					...
					if(empty($text_style)){
						$output .= '<p>';
						foreach($sector_client_array_2 as $sector_client):
							$output .= '<span class="clientList">' . $sector_client . '</span>';
					    endforeach;
						$output .= '</p>';
					}
                ...
                $output .= '	</div>';
				$output .= '</div>';
				$output .= '<div class="services-image-block">';        
				$output .= '<ul class="custom-block-container">';
				...
				foreach ($arrayRel as $value) {
					...
					$output .= '<li class="custom-block">';
					foreach ($imagesArray as $featuredImg) {
						if(empty($featuredImg["featured_image"])){
							if($readMore == 1){
								$output .= '<a href="' . $url . '">';
							}
							$output .= '<div class="projectHoverImg profile-box noImagePresent" style="background-color:rgba(191, 204, 215, 0.4);';
							if($readMore == 1){
								$output .= 'cursor:pointer;';
							} 
							$output .= '">';
					        $output .= '<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width: 100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>';
							$output .= '<div class="custom-block-text-container">';
							$output .= '	<h5 class="peopleText" style="font-weight: 600; color: #2b92b5; margin: 12px 20px 0px; font-size: 14px; text-transform: capitalize; display: block;">' . $client. '</h5>';
							$output .= '	<h6 class="peopleText" style="color: #2b92b5; margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px; padding-right: 10px; display:block; text-transform: capitalize; font-weight: 200; text-overflow: ellipsis;">' . $title . '</h6>';
							$output .= '</div>';
							if($readMore == 1){
								$output .= '<div class="peopleText read-more" style="position: absolute; bottom: 13px; color: #2b92b5; font-size: 12px; text-transform: capitalize; display: block; font-family: museo-sans, sans-serif; font-weight: 300;">Read more</div>';			
							}
							$output .= '</div>';
							if($readMore == 1){
								$output .= '</a>';
							...
							$output .= '<div class="projectHoverImg profile-box" style="position:relative;float:left;background:url(' . $featuredImg["featured_image"] . ') 50% 50% no-repeat;'; 
							if($readMore == 1){
								$output .= 'cursor:pointer;';
							}; 
							$output .= '">';
							...
							$output .= '<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width: 100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>';
							$output .= '	<div class="custom-block-text-container">';
							$output .= '		<h5 class="peopleText" style="font-weight: 600; color: rgb(255, 255, 255); margin: 12px 20px 0px; font-size: 14px; text-transform: capitalize; display: none;">' . $client . '</h5>';
							$output .= '		<h6 class="peopleText" style="color: rgb(255, 255, 255); margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px;  padding-right: 10px; display: none; text-transform: capitalize; font-weight: 200; text-overflow: ellipsis;">' . $title . '</h6>';
							$output .= '</div>';
							if($readMore == 1){
								$output .= '<div class="peopleText read-more" style="position: absolute; bottom: 13px; color: rgb(255, 255, 255); font-size: 12px; text-transform: capitalize; display: none; font-family: museo-sans, sans-serif; font-weight: 300;">Read more</div>';
							}
							$output .= '</div>';
							if($readMore == 1){
								$output .= '</a>';
				...
				$output .= '</ul>';
				$output .= '		<p class="view-all-link align-right">VIEW ALL <a href="' . site_url() . '/projects/?projects=' . $relLink .  '">PROJECTS  OF THIS TYPE</a></p>';
				$output .= '	</div>'; //-- end three project layout-->
				$output .= '</div>';
...
function custom_services_section_function(){
	$output = 'test';
	$args = array(
		'post_type' => 'page',
		'page_id' => 27,
	...
	//query_posts( 'page_id=27' );
	$query = new WP_Query( 'page_id=27'); 
	//if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); 
	//$query = new WP_Query( 'page_id=27' );
	
	$output = '';
    $output .= '<div id="expertise">';
	
	while ( $query->have_posts() ) : $query->the_post(); 
    //while ( have_posts() ) : the_post(); 
	//$output .= '<div class="heroText">' . get_the_content() . '</div>';
    if( get_field('aoe') ){
	...
	$output .= '<div class="services-block-container">';
	$output .= '<div style="" class="services-block">';
		$output .= '<div style="float:left; width:100%">';
			$output .= '<p><a href="/" class="back-to-top-btn">Back to Top</a></p>';
			...
			$output .= '<div class="block-first-col">';
			if(empty($heading_style)){
				$output .= '<h3 class="heading">' . $aoe_title . '</h3>';
			}
			else{
				$output .= '<h3 class="customH heading">' . $aoe_title . '</h3>';
			}
			$output .= '</div>';
			$output .= '<div class="services-description-container">';
				$output .= '<div class="services-description-column">';
				if(empty($text_style)){
					$output .= '<h5 style="color: rgb(103, 103, 103); text-transform: uppercase; font-weight: bold; line-height: 16px; font-size: 16px;">';
					foreach($aoe_description_array_1 as $aoe_description_item):
					$output .= '<span class="aoeList">' . $aoe_description_item . '</span>';
					endforeach;
					$output .= '</h5>';
				}else{
					$output .= '<' . $text_style . ' class="customH">';
				...
				$output .= '</div>';
			$output .= '</div>';
			if ($aoe_title=="LEED Services"){
				$relLink="leed-services";
			}elseif ($aoe_title=="Energy, Waste, &amp; Water"){
				$relLink="energy-waste-water";
			}elseif ($aoe_title=="Planning &amp; Infrastructure"){
				$relLink="planning-infrastructure";
			}elseif ($aoe_title=="Advanced Green Buildings"){
				$relLink="advanced-green-buildings";
			}elseif ($aoe_title=="General Green Building Consulting"){
				$relLink="other-green-building-consulting";
			...
			}elseif ($aoe_title=="Workshops &amp; Professional Education"){
				$relLink="workshops-professional-education";
			}elseif ($aoe_title=="Green Building Consulting"){
				$relLink="other-green-building-consulting";
			}else{
	...
	$output .= '<div class="services-image-block">';
		...
		foreach ($arrayRel as $value) {
			...
			if(empty($imagesArray[0]["featured_image"])){
		        ...
		        $output .= '<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width: 100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>';
				$output .= '<div class="custom-block-text-container">';
					$output .= '<h5 class="peopleText" style="font-weight: 600; color: #2b92b5; margin: 12px 20px 0px; font-size: 14px; display: block;">' . $client . '</h5>';
					$output .= '<h6 class="peopleText" style="color: #2b92b5; margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px;  padding-right: 10px; display:block; font-weight: 200; text-overflow: ellipsis;">' . $title . '</h6>';
				$output .= '</div>';  
				if($readMore == 1){
					$output .= '<div class="peopleText read-more" style="position: absolute; bottom: 13px; color: #2b92b5; font-size: 12px; display: block; font-family: museo-sans, sans-serif; font-weight: 300;">Read more</div>';
				}	
				$output .= '</div>';
				if($readMore == 1){
					$output .= '</a>';
				...
				foreach ($imagesArray as $featuredImg) {
					$output .= '<div class="projectHoverImg profile-box" style="position:relative;'; 
					if($readMore == 1){
						$output .= 'cursor:pointer;';
					}
					$output .= 'background:url(' . $featuredImg["featured_image"] . ') 50% 50% no-repeat;">';
				}
				$output .= '<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width: 100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>';
				$output .= '<div class="custom-block-text-container">';
				$output .= '<h5 class="peopleText" style="font-weight: 600; color: rgb(255, 255, 255); margin: 12px 20px 0px; font-size: 14px; display: none;">' .  $client . '</h5>';
					$output .= '<h6 class="peopleText" style="color: rgb(255, 255, 255); margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px;  padding-right: 10px; display: none; font-weight: 200; text-overflow: ellipsis;">' . $title . '</h6>';
				$output .= '</div>';
				$readMore = get_field('read_more', $projectID);
				if($readMore == 1){
					$output .= '<div class="peopleText read-more" style="position: absolute; bottom: 13px; color: rgb(255, 255, 255); font-size: 12px; display: none; font-family: museo-sans, sans-serif; font-weight: 300;">Read more</div>';
		...
		$output .= '</div>';
			...
			}else{  //end of $layoutstyle "three images or four images"
			}
    	} 
	}
        
	endwhile; 
    $output .= '</div>';//-- #expertise -->
	return $output;	
}


function custom_contact_section_function(){
	$output = '';
	global $post;
	$office = new WP_Query();
	...
	$output .= '<a name="' . strtolower($post->post_name) . '"></a>';
	$output .= '<div class="offices-block-container" id="'. strtolower($post->post_name) . '-office" style="margin:30px auto 0 auto;width:100%;position:relative;font-family: Open Sans;">';
		$output .= '<div style="float:left; width:100%">';
			$output .= '<div class="offices-block">';
				$output .= '<div class="offices-header-info-container">';
					$output .= '<h3>' . get_the_title() . '</h3>';
					$output .= '<h5 style="color: rgb(103, 103, 103); text-transform: uppercase; font-weight: bold; line-height: 18px; font-size: 15px;">' . get_field('subtitle') . '</h5>';
					$output .= '<p class="office-address">' . get_field('address') . '<br/>' . get_field('city') .  ', ' . get_field('state') . ' ' . get_field('zip') . '</p>';
					$output .= '<ul style="office-location-info-list">';
        			
					$output .= '<li style="margin: 5px 0;"><a class="icon-phone" style="font-size: 16px; line-height:27px; color:#2b92b5; text-decoration:none;" href="tel:+1' . $phoneNo .'">' . get_field('phone') . '</a></li>';

					if(get_field('fax')){ 
					$output .= '<li style="margin: 5px 0;"><a class="icon-fax" style="font-size: 16px; line-height:27px; color:#676767;text-decoration:none;" href="fax:+1' . $faxNo . '">' . get_field('fax') . '</a></li>';
					} 
					
					$output .= '	<li style="margin: 5px 0;"><a class="icon-mail" style="font-size: 16px; line-height:27px; color:#2b92b5;text-decoration:none;" href="mailto:' . get_field('email') . '">' . get_field('email') . '</a></li>';
					$output .= '</ul>';
				$output .= '</div>';

				$output .= '<div class="offices-header-image-container">';
					if( !empty($headerImage)){
						$output .= '<img src="' . $headerImage['url'] . '" alt="' . $headerImage['alt'] . '" />';
					}else{
						//$output .= '<span class="alert">Location has no image assigned.</span>';
					}
				$output .= '</div>';
			$output .= '</div>';
			$output .= '<div class="offices-image-block">';
...
// 			//echo $key;
//
// 			setup_postdata($related_project);
// 			if( get_field('featured_image',$related_project->ID) ){
// 				while( has_sub_field('featured_image',$related_project->ID) ){
// 				$featured_image = get_sub_field('featured_image');
// 		   		$readMore = get_field('read_more', $related_project->ID);
// 				$output .= '<li class="custom-block">';
// 					if(empty($featured_image)){
//
// 						if($readMore == 1){
// 							$output .= '<a href="' . get_permalink($related_project->ID) . '">';
// 						}
// 						$output .= '<div id="' . $post->ID . '" class="projectHoverImg profile-box noImagePresent" style="width:100%;height:100%;position:relative;';
// 						if($readMore == 1){
// 							$output .= 'cursor:pointer;';
// 						}
// 						$output .= 'float:left;margin-left:30px;margin-bottom:30px;background-color:rgba(191, 204, 215, 0.4);">';
//
// 						$output .= '<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width:100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>';
// 							$output .= '<div class="custom-block-text-container">';
// 								$output .= '<h5 class="peopleText" style="font-weight: 600; color: #2b92b5; margin: 0px 20px 0px 20px; font-size: 14px; text-transform: capitalize; display: block;">' . get_field('client',$related_project->ID) . '</h5>';
// 								$output .= '<h6 class="peopleText" style="color: #2b92b5; margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px;  padding-right: 10px; display: block; font-weight: 200;text-overflow: ellipsis;">' . get_the_title($related_project->ID) . '</h6>';
// 						$output .= '</div>';
//
// 						if($readMore == 1){
// 							$output .= '<div class="peopleText read-more" style="position: absolute; bottom: 13px; color: #2b92b5; font-size: 12px; display: block; font-family: museo-sans, sans-serif; font-weight: 300;">Read more</div>';
...
// 							$output .= '<a href="' . get_permalink($related_project->ID) . '">';
// 						}
// 						$output .= '<div id="' . $post->ID . '" class="projectHoverImg profile-box" style="';
// 						if($readMore == 1){
// 							$output .= 'cursor:pointer;';
// 						}
// 							$output .= 'float:left;background:url(' . $featured_image . ') 50% 50% no-repeat;">';
//
// 						$output .= '<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width:100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>';
// 							$output .= '<div class="custom-block-text-container">';
// 								$output .= '<h5 class="peopleText" style="font-weight: 600; color: rgb(255, 255, 255); margin: 0px 20px 0px 20px; font-size: 14px; text-transform: capitalize; display: none;">' . get_field('client',$related_project->ID) . '</h5>';
// 								$output .= '<h6 class="peopleText" style="color: rgb(255, 255, 255); margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px;  padding-right: 10px; display: none; font-weight: 200;text-overflow: ellipsis;">' . get_the_title($related_project->ID) . '</h6>';
// 							$output .= '</div>';
//
// 							if($readMore == 1){
//
...
function custom_people_section_function(){
	$output = '';
	$output .= '<div>';
	
	$person = new WP_Query(array('post_type' => 'people', 'posts_per_page' => -1, 'meta_key' => 'order_number','orderby' => 'meta_value_num','order' => 'ASC')); 
		$counter = 0;
			...
			if($counter == 0){
				$output .= '<ul class="custom-block-container">'; 
					...
					$output .= '<li class="custom-block">';
						...
						$output .= '<div id="' . get_the_ID() . '" class="profile-box';
							 if(empty($photo)){
								 $output .= ' noImagePresent';
							 } 
							 $output .=  ' list-item-' . $counter .'" style="';
						 if(empty($photo)){
							 $output .= 'background-color: rgba(191, 204, 215, 0.4);';
						 }else{
							 $output .= 'background-image:url(' . $photo . ');';
						 }
						 $output .= '">';
						 $output .= '<div style="background-color:#255280;opacity:0.8;position:absolute;bottom:0;height:100%;width:100%;display:none;" class="profile-box-highlight"></div>';
						 $output .= '<div class="custom-block-text-container">';
						 $output .= '	<h5 class="open-sans peopleText" style="font-weight:700;'; 						 						 if(empty($photo)){
							 $output .= 'color: #2b92b5;';
						 }else{
							 $output .= 'display:none;color:#fff;';
						 } 
						 $output .= 'margin:12px 0 0 20px;font-size:14px;">' . $name  . '</h5>';
						 $output .= '<h6 class="peopleText" style="margin:0 20px 0 20px;font-size:12px;letter-spacing:.5px;text-transform:uppercase;font-weight: 300;';
						 if(empty($photo)){
							 $output .= 'color: #2b92b5;';
						 }else{
							 $output .= 'display:none;color:#fff;';
		                 ...
		                 $output .= '<div class="peopleText locationText" style="padding:20px;bottom:13px;font-size:12px;font-family: museo-sans, sans-serif;font-weight:300;padding-top:8px;';
						 if(empty($photo)){
							 $output .= 'color: #2b92b5;';
						 }else{
							 $output .= 'display:none;color:#fff;';
						 ...
						 $output .= '">';
							 foreach($officeDetail as $officeInfo){
								 $output .= $officeInfo->post_title; 
								 if ($officeInfo != end($officeDetail)){
									 $output .= ' | ';
								 }
							 }
							 $output .= '</div>';
						 $output .= '</div>';
						 $output .= '<div class="peopleText read-more" style="font-weight:300;position:absolute;bottom:13px;font-size:12px;font-family: museo-sans, sans-serif; font-weight: 300;';
		 				 if(empty($photo)){
							 $output .= 'color: #2b92b5;';
						 }else{
							 $output .= 'display:none;color:#fff;';
						 } 
						 $output .= '">Read more</div>';
						 $output .= '</div>';
		                 $output .= '</a>';
...
function custom_project_blocks_function() {
	$output = '';

	$output .= '<div id="front-page-boxes" class="fpb1">';
				...
				while( has_sub_field($image) ){
					...
					if ($counter++ <= 8) {  
						...
						if (!$featured_image){
							...
							$output .= '<li class="custom-block"><a href="' . $permalink . '">';
							$output .= '<div class="fpbNoImageBox profile-box noImagePresent">'; //fpb-box
							$output .= '	<div class="fpb-shade fpbNoImageShade"></div>';
							$output .= '	<div class="profile-box-highlight"></div>'; //fpb-highlight fpbNoImageHighlight
					        $output .= '	<div class="custom-block-text-container fpbNoImageText">'; //fpb-text
				            ...
				            //apply preferred heading style
				            if(empty($heading_style)){
				                $output .= '<h5 class="peopleText" style="color: rgb(0, 143, 180); display: block;">' . $hoverTitle . '</h5>';  //fpb-header
				            }else{
				                $output .= '<' . $heading_style . ' class="peopleText" style="color: rgb(0, 143, 180); display: block;">' . $hoverTitle . '</' . $heading_style . '>'; //fpb-header
				            }
				            if(empty($text_style)){
				                $output .= '<h6 class="peopleText" style="color: rgb(0, 143, 180); display: block;">' . $hoverText. ' &nbsp;</h6>'; //fpb-subline
				            }else{
				                $output .= '<' . $text_style . ' class="peopleText" style="color: rgb(0, 143, 180); display: block;">' . $hoverText. ' &nbsp;</' . $text_style . '>'; //fpb-subline
				            }
							...
							if(1==1){//left here in case it is to be reused
								$output .= '<div class="fpb-readMore fpbNoImageReadMore peopleText" style="position: absolute;  bottom: 10px; font-size: 14px; text-transform: capitalize; display: block; font-family: museo-sans, sans-serif; font-weight: 300; color:#fff; display:none;">Read more</div>';
							...
							$permalink = get_permalink($post->ID);			
							$output .= '<li class="custom-block"><a href="' . $permalink . '"><div class="profile-box" style="background-image:url('.  $featured_image . ');">'; //fpb-box
					        $output .= '<div class="fpb-shade"></div>';
					        $output .= '<div class="profile-box-highlight"></div>'; //fpb-highlight
					        $output .= '    <div class="custom-block-text-container"> ';   //fpb-text 
                  		
							$heading_style = get_field('home_page_hover_title_style_project');
							$text_style = get_field('home_page_hover_text_style_project');
							//apply preferred heading style
							if(empty($heading_style)){
								$output .= '<h5 class="peopleText" style="display:none;">' . $hoverTitle . '</h5>'; //fpb-header
							}else{
								$output .= '<' . $heading_style . ' class="peopleText" style="display:none;">' . $hoverTitle . '</' . $heading_style . '>';//fpb-headerx
							}
							if(empty($text_style)){
								$output .= '<h6 class="peopleText" style="display:none;">' . $hoverText. ' &nbsp;</h6>'; //fpb-subline
							}else{	
								$output .= '<' . $text_style . ' class="peopleText" style="display:none;">' . $hoverText. ' &nbsp;</' . $text_style . '>'; //fpb-subline
							}
						
							$output .= '</div>';
							$readMore = get_field('read_more');
							if(1 == 1){ //all should have "read more". Left here in case of reuse.
							$output .= '<div class="fpb-readMore peopleText" style="position: absolute;  bottom: 10px; font-size: 14px; text-transform: capitalize; display: block; font-family: museo-sans, sans-serif; font-weight: 300; display:none; color:#fff;">Read more</div>';
							}
							$i++;				
							$output .= '</div>';
			        		$output .= '</a></li>';			
			...
			if($type == "people"){
				...
				if ($counter++ <= 8) {  
					if (!$featured_image){
					    ...
					    $output .= '<li class="custom-block"><a href="' . get_permalink()  . '">';
					    $output .= '<div class="fpbNoImageBox noImagePresent profile-box">'; //fpb-box 
					    $output .= '    <div class="fpb-shade fpbNoImageShade"></div>';
					    $output .= '    <div class="profile-box-highlight"></div>';//fpb-highlight fpbNoImageHighlight
					    $output .= '    <div class="custom-block-text-container fpbNoImageText">'; //fpb-text
						...
						$permalink = get_permalink($post->ID);
						$output .= '<li class="custom-block"><a href="' . $permalink . '"><div class="profile-box" style="background-size:cover; background-image:url(' .  $featured_image . ');">';	//fpb-box
						$output .= '<div class="fpb-shade"></div>';
						$output .= '<div class="profile-box-highlight"></div>'; //fpb-highlight
						$output .= '	<div class="custom-block-text-container">'; //fpb-tex
						$heading_style = get_field('home_page_hover_title_style_people');
						$text_style = get_field('home_page_hover_text_style_people');
						//apply preferred heading style
						if(empty($heading_style)){
							...
							$output .= '<' . $heading_style . ' class="peopleText" style="display:none;">' . $hoverTitle . '</' . $heading_style . '>'; //fpb-header
						}
						if(empty($text_style)){
							$output .= '<h6 class="peopleText" style="display:none;">' . $hoverText. ' &nbsp;</h6>'; //fpb-subline
						}else{
				...
				$output .= '</div>';
	                    ...
	                    if(1==1){//left here in case it is to be reused
	                    $output .= '<div class="fpb-readMore peopleText" style="position: absolute;  bottom: 10px; font-size: 14px; text-transform: capitalize; display: block; font-family: museo-sans, sans-serif; font-weight: 300; color:#fff; display:none;">Read more</div>';
...
function custom_project_page_function() {
	$i=0;
	$projectCounter = 0;
	...
	global $post; 
	$output = '';
	$output .= '<div id="loadingContainer"></div>';
		$output .= '<div id="content-area" class="functions2">';
			$output .= '<div class="project-page-container">';
			   
			   
			   //<!-- START project-filters -->
				$output .= '<div id="project-filters-container">';
				$output .= '<div class="fo">';
					$output .= '<div id="project-filters">';
		                $output .= '<ul>';
				        	$output .= '<li class="industry-sector-filter"><span class="li-title">Industry Sector</span>';
				             	$output .= '<ul id="industry-sector-filter">';  
		                        $industry_sectors = get_terms('industry-sector', array(
									  'orderby'  => 'name',
									  'order'    => 'ASC'
								)); 
				                foreach ($industry_sectors as $industry_sector) {
									$output .= '<li><a class="click" name="' . $industry_sector->name. '" id="' . $industry_sector->slug . '" href="http://brightworks.net/dev/projects/?projects=' . $industry_sector->slug . '&type=industry-sector-filter">' . $industry_sector->name . '</a></li>';
				                }      
				                $output .= '</ul>';
				                $output .= '<div class="moreIndicator" style="height:20px;text-align:center;background-color:#fff;display:none;"><i class="fa fa-angle-down"></i></div>';
				            $output .= '</li>';
				            $output .= '<li class="service-filter"><span class="li-title">Service</span>';
				            	$output .= '<ul id="service-filter">';
			                    ...
			                    foreach ($services as $service) {
									if ((!strpos($service->name,';')) && (!strpos($service->name,'/')) && (!strpos($service->name,'('))) { 
			                    		$output .= '<li><a class="click" name=" ' . $service->name . '" id="show-' . $service->slug . '" href="http://brightworks.net/dev/projects/?projects=' . $service->slug . '&type=service-filter">' . $service->name . '</a></li>';
									}else{ 
			                			$output .= '<li><a class="click" name="' . $service->name . '" id="show-' . $service->slug . '" href="http://brightworks.net/dev/projects/?projects=' . $service->slug . '&type=service-filter">' . $service->name . '</a></li>';
			               			} 
								} 
								...
								$output .= '<div class="moreIndicator" style="height:20px;text-align:center;background-color:#fff;display:none;"><i class="fa fa-angle-down"></i></div>';
							$output .= '</li>';
		                    $output .= '<li class="building-type-filter"><span class="li-title">Building Type</span>';
		                    	$output .= '<ul id="building-type-filter">';
		                        $building_types = get_terms('building-type', array(
									'orderby'  => 'name',
									'order'    => 'ASC'
								)); 
		                        foreach ($building_types as $building_type) {
		                            $output .= '<li><a class="click" name="' .$building_type->name . '" id="show-' . $building_type->slug . '" href="http://brightworks.net/dev/projects/?projects=' . $building_type->slug . '&type=building-type-filter">' . $building_type->name . '</a></li>';
		                        }
		                    	$output .= '</ul>';
		                    $output .= '<div class="moreIndicator" style="height:20px;text-align:center;background-color:#fff;display:none;"><i class="fa fa-angle-down"></i></div>';
		                    $output .= '</li>';
		        		    $output .= '<li class="location-filter"><span class="li-title">Location</span>';
			                	$output .= '<ul id="location-filter">'; 
		                        $locations = get_terms('location', array(
									'orderby'       =>  'term_order',
		                			'hide_empty'    => false
		  					  	)); 
			                    foreach ($locations as $location) {
			                             $output .= '<li><a class="click" name="' . $location->name . '" id="show-' . $location->slug . '" href="http://brightworks.net/dev/projects/?projects=' . $location->slug . '&type=location-filter">' . $location->name .  '</a></li>';
			                    }
			                    $output .= '</ul>';
								$output .= '<div class="moreIndicator" style="height:20px;text-align:center;background-color:#fff;display:none;"><i class="fa fa-angle-down"></i></div>';
							$output .= '</li>';
		                    $output .= '<li id="see-all">';
		                    	$output .= '<a href="' . site_url() . '/projects">See All</a>';
		                    $output .= '</li>';
			            $output .= '</ul>';
                	$output .= '</div>';
				$output .= '</div>';
                ...
                $output .= '<div id="project-listing" class="';
					if($_GET['view']) { 
						$output .= 'all';
					}
				$output .= '">';
      				$output .= '<div id="no-results">';
      					$output .= '<p>Sorry, there were no matches.  Please <a href="' . site_url() . '/projects">try again</a>.</p></div>';
						
					
				$output .= '<h1 style="color:#003260;" id="featuredProjectsTitle" class="projectsTitles">' . get_field('signature_projects_title') . '</h1>';
				...
				// START unfiltered	
	        	$output .= '<div id="unfilteredDiv" class="';
				if($qsCheck=="yes" && $qsValid == "yes"){
					$output .= 'hideThis';
				}else{
					$output .= 'showThis';
				}
				...
				if((get_field('signature_project') != "Signature Projects") && (get_field('signature_project') != "Signature Projects ") && $otherCheck == 0 && $i!=0) {
					$i = 0;
					$counter = 0;
					$output .= '<div class="collectedProjects" id="projectCollector' . $projectCollectorDivCounter . '"></div>';
					$projectCollectorDivCounter++;
				}
				if((get_field('signature_project') != "Signature Projects") && (get_field('signature_project') != "Signature Projects ") && $otherCheck == 0){
							$output .= '</ul><h1 style="color:#003260" class="projectsTitles">Other Projects</h1>';
				}
				if($counter == 0){
					$output .= '<ul class="custom-block-container othe-' . $otherCheck . '">';
				}
						
	                	$output .= '<div style="width:100%; display:none;" id="' .  $post->ID . '-detail" class="project-section  project' . $projectCounter;
						if((get_field('signature_project') == "Signature Projects") || (get_field('signature_project') == "Signature Projects ")) { 
							$output .= ' signature-project ';
						}elseif((get_field('signature_project') != "Signature Projects") && (get_field('signature_project') != "Signature Projects ") && $otherCheck == 0){
							$output .= ' firstOther other-project '; 
							$otherCheck = 1;
						}else{
					...
					foreach ($building_types as $building_type) {
						...
						$output .= ' show-'.$building_type_slash;
						array_push($classArray, ' show-'.$building_type_slash);
					} 
					$output .= '">';
	                 $output .= '<div class="project-left yessir4" style="width:48%">';
									$output .= '<h2>';
									$output .= get_field('client');
									$output .= '</h2>';
					                $output .= '<h1>';
									if($readMore == 1){
					                	$output .= '<a class="readMoreIndicator" href="' . get_permalink() . '">' . get_the_title() . '</a>';
									}else{
										$output .= get_the_title();
									} 
									$output .= '</h1>';
									$output .= '<span style="float:left;clear:both;" class="clientLoc">'; 							
									if(get_field('city')) { 
										$output .= get_field('city');  
									} 
									if(get_field('city') && get_field('state')) { 
										$output .= ', '; 
									} 
					                if(get_field('state')) { 
										$output .= get_field('state'); 
									}  
									$output .= '</span>';
				                    $output .= '<div class="clear"></div>';		
						...
						$serviceArray=array();	
						if($services){
		                    $output .= '<ul class="project-services">';
	                        $output .= '<li class="title-li">Services provided</li>';
	                        foreach ($services as $service) { 
	                        	$cleaned = str_replace('&amp;','&',$service->name);
	            				$pieces = explode("; ", $cleaned);
								foreach($pieces as $element) { $serviceArray[] = $service->name;
	                            $output .= '<li class="bg-fill"><a class="click" name="' . $service->name . '" id="show-' . $service->slug . '" href="http://brightworks.net/dev/projects/?projects=' . $service->slug . '&type=service-filter">' . $element . '</a></li>';
								} 
							}    
		       				$output .= '</ul>';
						}
						...
						$industry_sectors = wp_get_post_terms($post->ID, 'industry-sector', array("fields" => "all"));
						if($industry_sectors){
		                    $output .= '<ul class="project-sectors">';
	                        $output .= ' <li class="title-li" >Industry sector</li>';
	                        foreach ($industry_sectors as $industry_sector) { 
	                            $output .= '<li class="bg-fill"><a class="click" name="' . $industry_sector->name . '" id="show-' .$industry_sector->slug . '" href="http://brightworks.net/dev/projects/?projects=' . $industry_sector->slug . '&type=industry-sector-filter">' . $industry_sector->name . '</a></li>';
	                        }
	                    	$output .= '</ul>';
						}
                    	$output .= '<div class="clear"></div>';
						$building_types = wp_get_post_terms($post->ID, 'building-type', array("fields" => "all"));
						if($building_types){
							$output .= '<ul class="building-types">';
	                        $output .= '<li class="title-li" >Building types</li>';
							$building_types = wp_get_post_terms($post->ID, 'building-type', array("fields" => "all"));
	                        foreach ($building_types as $building_type) {
	                            $output .= '<li class="bg-fill"><a class="click" name="' . $building_type->name . '" id="show-' . $building_type->slug .'" href="http://brightworks.net/dev/projects/?projects=' . $building_type->slug . '&type=building-type-filter">' . $building_type->name . '</a></li>';
							}
		                    $output .= '</ul>';
						}
	                    $output .= '</div>';//<!-- .project-left yessir -->
						//<!-- .project-middle -->
						if( get_field('featured_image') ){
							while( has_sub_field('featured_image') ){ 
								$featured_image = get_sub_field('featured_image');
								...
								if ($featured_image) {  
				                    ...
				                    $output .= '<div class="project-right blah" style="width:48.8%; height: 100%; float: right;  background-image: url(' . $featured_image . '); background-position: 50% 50%; background-repeat: no-repeat; background-size:cover;"><img src="' . $featured_image . '" />';
				                    $output .= '</div>';
				                    if($readMore == 1){
										$output .= '</a>';
									}
						...
						if(empty($featured_image)){
							$backgroundStyle = 'background-color:rgba(191, 204, 215, 0.4);'; 
							$imageCheck = "noImagePresent";
						}else{
							$backgroundStyle = 'background:url(' . $featured_image . ') 50% 50% no-repeat;'; 
							$imageCheck = "imagePresent";}
							$output .= '<li class="custom-block"><div id="' . $post->ID . '" class="projectHoverImg profile-box'; 
							foreach($classArray as $value){
								$output .= $value;
							}
							$output .= ' ' . $imageCheck . '"'; 								
							$output .= 'style="' . $backgroundStyle . '">';
							$output .= '<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width:100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>';
								$output .= '<div class="custom-block-text-container">';
									$output .= '<h5 class="peopleText" style="font-weight: 600; color: rgb(255, 255, 255); margin: 12px 20px 0px 20px; font-size: 14px;display: none;">' . $clientName . '</h5>';
									$output .= '<h6 class="peopleText" style="color: rgb(255, 255, 255); margin: 0px 0px 0px 20px; font-size: 13px; letter-spacing: 0.5px;  padding-right: 10px; display: none;font-weight: 200;text-overflow: ellipsis;">' . get_the_title() . '</h6>';
						...
						$i++;
						if($i==4){
						$output .= '<div class="collectedProjects" id="projectCollector' . $projectCollectorDivCounter . '" style=""></div>';
						$i=0;
						$projectCollectorDivCounter++;
						}
					...
					$postCounter = 0;
					$counter2 = 0;
					$output .= '<div id="filteredServicesDiv" class="';
					if($qsCheck=="yes" && $qsValid == "yes"){
						$output .= 'showThis';
					}else{
						$output .= ' hideThis';
						...
						while ( $projects->have_posts() ) :
							...
							foreach ($services as $service) {
								...
								if($service_amp == $selectedFilter && $qStringType == "service-filter"){ 
				    				...
				    				if((get_field('signature_project') != "Signature Projects") && (get_field('signature_project') != "Signature Projects ") && $otherCheckF == 0 && $i!=0) {
										$output .= '</ul>';
									    $output .= '<div class="collectedProjectsF" id="projectCollectorF' . $projectCollectorDivCounter . '"></div>';
										$output .= '<h1 style="color:#003260;" id="featuredProjectsTitle" class="projectsTitles">Other Projects</h1><ul class="custom-block-container">';
										$projectCollectorDivCounter++;
										$i=0;
				    				}
				                	$output .= '<div style="width:100%; display:none;" id="' . $post->ID . 'F-detail" class="project-section projectF' . $projectCounter . ' ';
									if((get_field('signature_project') == "Signature Projects") || (get_field('signature_project') == "Signature Projects ")) { 
										$output .= 'signature-project '; 
										$signatureCheck = 1;
									}elseif((get_field('signature_project') != "Signature Projects") && (get_field('signature_project') != "Signature Projects ") && $otherCheckF == 0){
										$output .= 'firstOtherF  other-projectF '; 
										$otherCheckF = 1;
									}else{
										$output .= 'other-projectF ';
									} 
									$output .= 'projectF';
									if( get_field('featured_image') ){
										while( has_sub_field('featured_image') ){ 
									...
									if( get_field('featured_image') ){
										while( has_sub_field('featured_image') ){ 
											...
											if ($featured_image) {  
												...
												$output .= '<div class="project-right" style="width:48.8%; height: 100%; float: right; background-image: url(' . $featured_image . '); background-position: 50% 50%; background-repeat: no-repeat; background-size:cover;"></div>';
									            if($readMore == 1){
													$output .= '</a>';
												} 
									                    // project-right 
									...
									$theContent = $theContent[0] . '...';
									if(empty($featured_image)){
										$backgroundStyle = 'background-color:rgba(191, 204, 215, 0.4);'; $imageCheck = "noImagePresent";
									}else{
										$backgroundStyle = 'background:url(' . $featured_image . ') 50% 50% no-repeat;'; 														$imageCheck = "imagePresent";
									}
									$output .= '<li class="custom-block"><div id="' . $post->ID . 'F" class="projectHoverImg profile-boxF'; 
									foreach($classArray as $value){
										$output .= $value;
									}
									$output .= ' ' . $imageCheck . '" style="' . $backgroundStyle . '">';
							...
							foreach ($industry_sectors as $industry_sector) {
								...
								if($industry_amp == $selectedFilter && $qStringType == "industry-sector-filter"){
										...
										while( has_sub_field('featured_image') ){ 
											...
											if ($featured_image) {  
									        	if($readMore == 1){
									          	...
									          	$output .= '<div class="project-right" style="width:48.8%; height: 100%; float: right; background-image: url(' . $featured_image . '); background-position: 50% 50%; background-repeat: no-repeat; background-size:cover;">';
									            $output .= '</div>';
									            if($readMore == 1){
													$output .= '</a>';
												}
									...
									$output .= '<li class="custom-block"><div id="' . $post->ID . 'F" class="projectHoverImg profile-boxF'; 													foreach($classArray as $value){
										$output .= $value;
									}
									$output .= ' ' . $imageCheck . '" style="' . $backgroundStyle . '">';
									$output .= '<div style="opacity: 0.8; position: absolute; bottom: 0px; height: 100%; width:100%; display: none; background-color: rgb(37, 82, 128);" class="profile-box-highlight"></div>';
							...
							foreach ($building_types as $building_type) {
							    ...
							    //put remaining projects in collector div, and start a new one. For break because of  "other projects" title.
								    ...
								    $output .= '<div style="width:100%; display:none;" id="' . $post->ID . 'F-detail" class="project-section projectF' . $projectCounter . ' '; 
									if((get_field('signature_project') == "Signature Projects") || (get_field('signature_project') == "Signature Projects ")) { 
										$output .= 'signature-project '; $signatureCheck = 1;
									}elseif((get_field('signature_project') != "Signature Projects") && (get_field('signature_project') != "Signature Projects ") && $otherCheckF == 0){
										$output .= 'firstOtherF  other-projectF '; 
										$otherCheckF = 1;
									}else{
					...
					$output .= '</div>'; //<!-- END filtered Div -->
				$output .= '</div>'; // <!-- END project-listing -->	
			$output .= '<br style="clear:both;"/>';
		$output .= '</div>';  // END project-page-container
	$output .= '</div>'; // END content-area
	//$output .= $signatureCheck; 
	if($signatureCheck == 0 && $qsCheck=="yes"){
		$output .= '<style>.projectsTitles{display:none!important;}</style>';
	}
	$output .= '<span class="hide" id="nonSigTitle">' . get_field('non-signature_projects_title') . '</span>';

	return $output;
}


add_shortcode('custom_project_page', 'custom_project_page_function');
add_shortcode('custom_project_blocks', 'custom_project_blocks_function');
add_shortcode('custom_services_section', 'custom_services_section_function');	
add_shortcode('custom_people_section', 'custom_people_section_function');
add_shortcode('custom_contact_section', 'custom_contact_section_function');
add_shortcode('custom_clients_section', 'custom_clients_section_function');

	
//--> END - CUSTOM BRIGHTWORKS FUNCTIONS <--//