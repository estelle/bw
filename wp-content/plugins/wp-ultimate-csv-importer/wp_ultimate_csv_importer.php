...
{
    add_menu_page('CSV importer settings', 'CSV Importer', 'manage_options', 'upload_csv_file', 'upload_csv_file', WP_CONTENT_URL . "/plugins/wp-ultimate-csv-importer/images/icon.png");
}
			...
			?>
				...
				if (($_REQUEST['action'] == 'post') || ($_REQUEST['action'] == 'custompost') || ($_REQUEST['action'] == 'page') || ($_REQUEST['action'] == 'users') || ($_REQUEST['action'] == 'comments')) {
							...
							?>
								...
								<?php } else if ($_REQUEST['action'] == 'post' || $_REQUEST['action'] == 'page') { ?>
									<input name='csv_importer_cat' type='hidden' id='csv_importer_cat'
										...
										style="display:none;">
										<span style="display: none; color: red; margin-left: 5px;" id="passwordhint">Replace the default value</span>
										</td>
										</tr>
										</table>
										<?php } ?>
                  ...
                  <select name="mapping<?php print($count); ?>"
                    ...
                    <option value="CF: client">Account Name</option>
                    <option value="post_title">Opportunity Name</option>
                    <option value="service">Service Category</option>
                    <option value="location">Region Where Located</option>
                    <option value="CF: city">City</option>
                    <option value="CF: state">State</option>
                    <option value="industry-sector">Primary Industry / Sector</option>
                    <option value="building-type">Primary Building Type</option>
                    <option value="post_content">Project Headline</option>
                    <option value="CF: certification_target">Certification Target</option>
                    <option value="CF: certification_level">Certification Level</option>
